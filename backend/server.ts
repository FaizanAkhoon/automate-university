...
// This middleware monkey-patches res.writeHead to FORCE CORS headers onto every
// single response, including those generated by Better Auth's toNodeHandler
// which normally overwrites Express middleware headers with its own.
// This is the only reliable way to guarantee CORS headers on Vercel serverless.
app.use((req: Request, res: Response, next: NextFunction) => {
  const origin = req.headers.origin;

  // Monkey-patch writeHead to inject CORS headers no matter what
  ...
  (res as any).writeHead = function (statusCode: number, ...args: any[]) {
    ...
    res.setHeader('Access-Control-Allow-Headers', 'Content-Type, Authorization, X-Requested-With, cookie, Cookie');
    res.setHeader('Access-Control-Expose-Headers', 'Set-Cookie');
    return originalWriteHead(statusCode, ...args);
  };

  // Also set them normally for non-Better-Auth routes
  if (origin) {
    res.setHeader('Access-Control-Allow-Origin', origin);
    res.setHeader('Vary', 'Origin');
  }
  ...
  // Immediately respond to OPTIONS preflight — don't let it reach Better Auth
